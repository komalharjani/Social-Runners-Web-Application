 ...
 * Route to Fetch Runs
 */
app.get('/getRuns/:type', function (request, response) {
	let type = request.params.id; //"run"
	dbRun.getRunsType(type)
		...
		.catch(err => {
			console.log(err);
			response.status(500).end(`Could not get Run with id ${type}`);
		});
});
...
/**
 * Get All Runs - this only displays on page
 */
// app.get('/getRuns/', function (request, response) {
// 	dbRun.getAll()
// 		.then(jsn => { 
// 			//let run = model.Run.fromJSON(jsn); // doesn't work because jsn is a bunch of runs
...
// 		.catch(err => {
// 			//console.log(err);
// 			response.status(500).end(`Could not get Runs`);
// 		});
// });
