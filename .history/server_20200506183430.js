...
var configRun = require('./config-runDb.js');
...
app.post('/usersPost', async (req, res) => {
	try {
		...
		const user = { name: req.body.name, email: req.body.email, password: hashedPassword, age: req.body.age, salt: salt }
		users.push(user);
		res.status(201).send();
	}
	catch {
...
var dbTwo = new dao.runDAO(configRun.db_info.url,configRun.db_info.username,configRun.db_info.password)
