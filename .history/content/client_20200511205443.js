...
setInterval(empty, 5000);
function empty() {
	document.getElementById("runPosts").empty();
	document.getElementById("left").empty();
	console.log("empty");
}
...
function generateSquares(runs, joiners, commentsData) {
	...
	if (arrayLength > 0) {
		for (let i = 0; i < arrayLength; i++) {
			...
			time.innerHTML = "At pace " + runs[i].pace + "it would take approximately " + runs[i].duration;
			table.append(time);
