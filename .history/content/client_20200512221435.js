...
function generateSquares(runs, joiners, commentsData) {
	...
	if (arrayLength > 0) {
		...
		for (let i = 0; i < arrayLength; i++) {
			...
			let distanceNum =  (runs[i].distance);
			let final = parseInt(distanceNum + 1);
			final = JSON.parse(distanceNum);
			...
			if (joiners.length > 0) {
				for (let j = 0; j < joiners.length; j++) {
					if (joiners[j].idRun == runID) { //if they haven't already joined
						...
						runsPerUserCount++;
						document.getElementById("noRunsJoin").innerHTML = "Number of runs joined: (excl. admin)" + runsPerUserCount;
						if(runs[i].status == "Completed") {
							totalMiles = totalMiles + final;
