...
async function getRunPosts() {
	//let poll = setInterval(async function () {
		...
		if (response.status == 200) {
			...
			generateSquares(getRuns, joiners, comments);
			final = [];
		}
//	}) //you're redrawing every 5 secs - wrong it is appending and redrawing instead of  
}
 ...
 * @param {*} comments 
 */
function generateSquares(getRuns, joiners, commentsData) {
	...
	if (arrayLength > 0) {
		for (let i = 0; i < arrayLength; i++) {
			...
			time.innerHTML = "At Pace: " + runs[i].pace + " miles/hour" + "it would take approximately " + runs[i].duration ;
			table.append(time);
