...
function generateSquares(runs) {
	...
	if (arrayLength > 0) {
		for (let i = 0; i < arrayLength; i++) {
			...
			let thisEmail = runs[i].email;
			let runId = runs[i].email;
			joinRun.onclick = joinRun(runId, thisEmail);
...
async function joinRun(runId, runOwnerEmail) {
	//clientEmail = user requesting to join
	//runOwnerEmail = user posted run
