			...
			if (joiners.length > 0) {
				...
				for (let x = 0; x < runs.length; x++) {
					for (let j = 0; j < joiners.length; j++) {
						//if joiners' email == this email - calculate how many runs and miles completed 
						//they have joined THIS RUN
						if (joiners[j].email == clientEmail) {
							//and if the status of the run is completed
							if (runs[x].status == "Completed") {
								countRuns++;
								document.getElementById("noRunsJoin").innerHTML = "Number of Runs Completed:" + countRuns;
								let distance = JSON.parse(runs[x].distance);
								console.log(runs[x].title);
								console.log(runs[x].distance);
								// totalMiles = parseInt(totalMiles + distance);
								totalMiles = JSON.parse(runs[i].distance) + totalMiles;
								document.getElementById("totalMiles").innerHTML = "Number of Miles Run: " + totalMiles;
							}
						}
					}
