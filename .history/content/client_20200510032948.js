...
function generateSquares(runs, joiners) {
	...
	if (arrayLength > 0) {
		for (let i = 0; i < arrayLength; i++) {
			...
			maxText.innerHTML = "Users Joined: ";
			table.append(maxText);

			let max = runs[i].maxPeople;
			let count = 0;
			let participantsForRun = [];

			//Function for rejecting "join Run on click
			//Another for loading previous joins
			...
			if (joiners.length > 0) {
				for (let j = 0; j < joiners.length; j++) {
					if (joiners[j].id == runID) { //if they haven't already joined
						count++;
						maxText.innerHTML.append(joiners[j].name);
						}
					}
				}
			}
			...
			joinRun.onclick = function () {
				...
				if (clientEmail == runOwnerEmail) {//or if they've joined before
					...
					alert ("Sorry! This run has reached it's maximum capacity");
				}
				else {
