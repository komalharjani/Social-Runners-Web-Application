...
async function getRunPosts() {
	...
	let joins = [];
	//getRuns
	let responseRuns = await fetch(`/getRuns/`);
	let getRuns = await responseRuns.json();
	console.log(getRuns);
	...
	// //getJoins
	// let responseJoins = await fetch(`/getJoins/`);
	// let getJoins = await responseJoins.json();
	// for (let i = 0; i < getJoins.length; i++) {
	// 	joins.push(getJoins[i].value);
	// }

	// console.log(joins);
	// let responseCom = await fetch(`/getComs/`);
	// let getComs = await responseCom.json();
	// for (let i = 0; i < getComs.length; i++) {
	// 	comments.push(getComs[i].value);
	// }

	//console.log(comments);
	generateSquares(runs, joins, comments);
}
	...
	if (runs.length > 0) {
		for (let i = 0; i < runs.length; i++) {
			...
			submit.onclick = async function () {
				...
				let comsId = (Math.random() * (50000 - 30000) + 1);
				...
				if (com != "" || com == "undefined") {
					let response = await fetch(`/addComment/${comsId}`, {
						method: 'POST',
						headers: { 'Content-Type': 'application/json' },
