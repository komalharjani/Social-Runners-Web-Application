...
function generateSquares(runs, joiners, commentsData) {
	...
	if (arrayLength > 0) {
		...
		for (let i = 0; i < arrayLength; i++) {
			...
			let finalDistance = distanceNum.substr(0,distanceNum.indexOf(' '));
				...
				for (let j = 0; j < joiners.length; j++) {
					if (joiners[j].idRun == runID) { //if they haven't already joined
						...
						if(runs[i].status == "Completed") {
							totalMiles += parseInt(finalDistance);
							console.log(finalDistance);
							document.getElementById("totalMiles").innerHTML = "Number of Miles Ran: " + totalMiles;
						}
