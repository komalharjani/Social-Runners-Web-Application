...
function generateSquares(runs, joiners, commentsData) {
	...
	function updateProfile(runs, joiners) {
		let thisUser = [];
		let totalMiles = 0;
		let count = 0;
		//this email
		...
		for (let i = 0; i < runs.length; i++) {
			for (let j = 0; j < joiners.length; j++) {
				//if the joiner's runID matches the Id of the run
				if (runs[i]._id == joiners[j].idRun) {
					//and the email of the joiner is this client
					if (joiners[j].email == thisClient) {
						count++
						document.getElementById("noRunsJoin").innerHTML = "Number of Runs Completed:" + count;
						let distance = JSON.parse(runs[i]._id);
						totalMiles = parseInt(totalMiles + distance);
						document.getElementById("totalMiles").innerHTML = "Number of Miles Run: " + totalMiles;
					}
				}
			}
			...
			if (joiners.length > 0) {
				for (let j = 0; j < joiners.length; j++) {
					if (runs[i]._id == joiners[j].idRun) {
						...
						if (joiners[j].email == thisClient) {
							runsPerUserCount++;
							document.getElementById("noRunsJoin").innerHTML = "Number of Runs Completed:" + runsPerUserCount;
							let distance = JSON.parse(runs[i]._id);
							totalMiles = parseInt(totalMiles + distance);
							document.getElementById("totalMiles").innerHTML = "Number of Miles Run: " + totalMiles;
						}
