...
app.post('/addJoin/:id', async (req, res) => {
	...
	.then(comment => {
		...
		if (comments.some(item => item.email === body.email && item.id === body._id)) {
			res.status(500).send(`You have already signed up for this run.`)
		}
		else {
			dbRun.insertRun(id, body)
		.then(joins => {
			joinsList.push(joins);
			console.log(joinsList);
			if (joinsList.some(item => item.email === body.email && item.id === body._id)) {
				res.status(500).send(`You have already signed up for this run.`)
			}
			else {
