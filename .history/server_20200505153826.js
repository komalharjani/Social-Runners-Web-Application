...
app.put('/addUser/:id', async (req, res) => {
	...
	const user = { name: req.body.name, password: hashedPassword, email: req.body.email, age: req.body.age, salt: salt }
...
app.put('/userLogin/:id', await (req, res) => {
	let id = req.params.id;
	db.getUser(id)
		.then(jsn => {
			console.log
			...
			let inputHash = await bcrypt.hash(inputpass, salt);
